...
// w_paste now uses click-to-confirm logic
...
function bwe_togglePanel() {
    ...
    bwe_panel.style.display = visible ? "none" : "block";
    if (!visible) bwe_renderPanel();
}
...
function bwe_renderPanel() {
    ...
    // ── Header
    let html = `<h3>🛠️ betterWorldEdit</h3>`;
    ...
    // Intercept copy/cut to create named slots
    // (actual slot creation is done in the override below)
    ...
    html += `<div class="bwe-section">
        ...
        <div class="bwe-label" style="font-size:10px;color:#888;">Click to anchor preview → click near same spot to confirm. Click far away to reposition.</div>
    </div>`;
...
// Mobile pointer events (kept from worldEdit.js)
