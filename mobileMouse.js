...
(function () {
    ...
    //  INIT
    //
    //  runAfterLoad() is unreliable for mods installed through the Mods panel:
    //  the game may have already flushed runAfterLoadList before this script
    //  executes, meaning the callback would never run.
    //
    //  Instead we poll every 100 ms until #canvasDiv appears in the DOM, then
    //  build the UI and install the touch blocker immediately.  This is safe
    //  to call at any time and requires no cooperation from the game's load
    //  sequencing.
    // ────────────────────────────────────────────────────────────────────────────
